...
// Pex integration by Marcin Ignac http://twitter.com/marcinignac, 2015-09

const float luminance = 1.0;
const float turbidity = 10.0;
const float raileigh = 2.0;
...
vec3 sky(vec3 sunPosition, vec3 worldNormal) {
  ...
  float raileighCoefficient = raileigh - (1.0 * (1.0 - sunfade));

  float sunE = sunIntensity(dot(sunDirection, up));

  // extinction (absorbtion + out scattering)
  // rayleigh coefficients
  vec3 betaR = totalRayleigh * raileighCoefficient;
  // vec3 betaR = simplifiedRayleigh * raileighCoefficient;
  ...
  vec3 LinFactor = sunE * ((betaRTheta + betaMTheta) / (betaR + betaM));
