...
// See standard.js for the shared Frame/Model/Material bind group and
// attribute @location conventions. This pass only needs base color + alpha
// to resolve opacity for alpha testing/texturing; the resolved base color
// itself is never used (DEPTH_PASS_ONLY skips writing it), only the packed
// view-space depth is output. The displacement offset here is stretched by
// 1.3x relative to standard.js's, matching the original GLSL depth pass, to
// reduce shadow acne/peter-panning from displaced surfaces.
...
override DEPTH_PASS_ONLY: bool = true;
...
fn fragmentMain(input: Varyings) -> FragmentOutput {
  ...
  output.color = packDepth(length(input.positionView) / DEPTH_PACK_FAR);
